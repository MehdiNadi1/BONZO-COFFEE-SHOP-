...
// Note: In Next.js, we need to handle this carefully as L might not be available on SSR
...
const SearchControl = ({ onSearch }: any) => {
  ...
  useEffect(() => {
    const control = new (L.Control as any)({ position: 'topleft' });
...
export const AdvancedMap = ({
  ...
  const handleToggleLayer = useCallback((layerType: string) => {
    setCurrentLayers(prev => ({
      ...prev,
      [layerType]: !(prev as any)[layerType]
    }));
  }, []);
    ...
    <div className={`advanced-map ${className}`} style={style}>
      ...
      >
                ...
                {marker.popup && (
                  <Popup>
                    <div style={{ color: 'black' }}>
                      <h3 style={{ margin: '0 0 5px 0' }}>{marker.popup.title}</h3>
                      <p style={{ margin: '0 0 10px 0' }}>{marker.popup.content}</p>
                      {marker.popup.image && (
                        <img 
            ...
            >
              {marker.popup && (
                <Popup>
                  <div style={{ color: 'black' }}>
                    <h3 style={{ margin: '0 0 5px 0' }}>{marker.popup.title}</h3>
                    <p style={{ margin: '0' }}>{marker.popup.content}</p>
                  </div>
                </Popup>
              )}
            </Marker>
        ...
        {userLocation && (
          ...
          >
            <Popup><span style={{ color: 'black' }}>Your current location</span></Popup>
          </Marker>
        )}
        ...
        {searchResult && (
          ...
          >
            <Popup><span style={{ color: 'black' }}>{searchResult.name}</span></Popup>
          </Marker>
        )}
        ...
        {polygons.map((polygon, index) => (
          ...
          >
            {polygon.popup && <Popup><span style={{ color: 'black' }}>{polygon.popup}</span></Popup>}
          </Polygon>
        ))}
        ...
        {circles.map((circle, index) => (
          ...
          >
            {circle.popup && <Popup><span style={{ color: 'black' }}>{circle.popup}</span></Popup>}
          </Circle>
        ))}
        ...
        {polylines.map((polyline, index) => (
          ...
          >
            {polyline.popup && <Popup><span style={{ color: 'black' }}>{polyline.popup}</span></Popup>}
          </Polyline>
        ))}
      </MapContainer>
    </div>
